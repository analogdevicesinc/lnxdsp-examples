...
#define ADI_RESOURCE_TABLE_SHARC1_OFFSET (0x800) //1KiB
#define ADI_VRING_BUFFER_SIZE 0x00400
...
struct rpmsg_lite_ept_static_context arm_ctrl_endpoint_context;
struct rpmsg_lite_ept_static_context arm_tuning_endpoint_context;

#define SHARC0_CTRL_EP_ADDR 160
#define SHARC0_TUNING_EP_ADDR 165
#define SHARC1_CTRL_EP1_ADDR 172

int32_t sharc1_to_shar0_ctrl(void *payload, uint32_t payload_len, uint32_t src, void *priv)
{
	return RL_RELEASE;
}

int32_t arm_to_sharc0_ctrl(void *payload, uint32_t payload_len, uint32_t src, void *priv)
{
	char ack[] = "ack";

	// forward message to SHARC1
	rpmsg_lite_send(
				&rpmsg_SHARC_channel,
				&sharc_ctrl_endpoint_context.ept,
	...
	rpmsg_lite_send(
				&rpmsg_ARM_channel,
				&arm_ctrl_endpoint_context.ept,
				src,
				ack, sizeof(ack), 0);
	return RL_RELEASE;
}

int32_t arm_to_sharc0_tuning(void *payload, uint32_t payload_len, uint32_t src, void *priv)
{
	char ack[] = "ack";
	...
	rpmsg_lite_send(
				&rpmsg_ARM_channel,
				&arm_tuning_endpoint_context.ept,
				src,
				ack, sizeof(ack), 0);
	return RL_RELEASE;
}
...
int rpmsg_init_channel_to_ARM(void)
{
	...
	rpmsg_ept = rpmsg_lite_create_ept(
			...
			&arm_to_sharc0_tuning,
			NULL,
			&arm_tuning_endpoint_context);
